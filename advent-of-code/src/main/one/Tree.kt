...
fun main() {
    val input = Elf::class.java.getResourceAsStream("input-03.txt")?.bufferedReader()?.readText()
    if (input != null) {
        println("Score: " + DayTree().getTotalPriority(input))
        println("Sum of all common items: " + DayTree().getPrioritySumOfAllCommonItems(input))
    }
}